...
class CBusCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry) -> None:
        ...
        self.monitoring_enabled = config_entry.options.get(CONF_MONITORING_ENABLED, True)
    ...
    async def async_ramp_device(self, group: int, level: int, ramp_time: int = 0) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Ramping group %s to level %s over %s seconds", group, level, ramp_time)
        except Exception as ex:
